...
extern void gethostname(); /* some platforms e.g. SunOS complain if this is not
			      declared */
  ...
  for(i=0; i<=argc; i++)
   ...
   /**************************************************/
	...
	case 'v': /* Print version number */
      printf("Sim v2.35, Released Sun, April 8 2001 \n");
      exit(0);
	case 'h':
...
char **argv;
{
  ...
  for(i=0;i<argc;i++)
    printf("%s ", argv[i]); 
  time(&aTime);
  printf("This is Sim version 2.35, Released Sun April 8th 2001 \n"); 
  printf("\n# Date %s", ctime(&aTime)); 
